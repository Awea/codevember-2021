...
const { range, chance } = require('canvas-sketch-util/random')
...
let angle = 30 // how much the turtle turns with a '-' or '+'
...
canvasSketch(() => {
  return {
    ...
    render({ context, width, height, time }) {
      ...
      // Try 3 times to find new coordinates
      let new_coordinates_attempts = 0
      let way = chance(0.5) ? 1 : -1

      while (outside(x, y) && new_coordinates_attempts < 4) {
        currentangle += way * angle
        x = prevX
        y = prevY
        lSystem(thestring[whereinstring])

        // console.log(`attempt: ${new_coordinates_attempts}`)

        new_coordinates_attempts++
      }
